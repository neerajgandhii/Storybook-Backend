...
def call_groq_with_retry(prompt: str, max_retries: int = 2) -> Optional[str]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Groq API call succeeded on attempt {attempt + 1}")
            return result
    ...
    print("ERROR: All Groq API attempts failed")
    return None
...
{{"rounds": [{{"id": "ai-4", "type": "text", "promptText": "Tap...", "items": ["sentence 1", "sentence 2", "sentence 3", "sentence 4", "sentence 5"]}}, {{"id": "ai-5", "type": "text", "promptText": "Tap...", "items": ["sentence 1", "sentence 2", "sentence 3", "sentence 4", "sentence 5", "sentence 6"]}}]}}"""
    ...
    print(f"[generate-rounds] Calling Groq API...")
    response_text = call_groq_with_retry(prompt)

    if not response_text:
        print(f"[generate-rounds] Groq API returned None, using fallback")
        return GenerateRoundsResponse(rounds=FALLBACK_ROUNDS, source="fallback")

    print(f"[generate-rounds] Groq response received, length={len(response_text)}")
    ...
    if "rounds" not in parsed or len(parsed["rounds"]) < 2:
        print(f"[generate-rounds] Invalid response structure")
        return GenerateRoundsResponse(rounds=FALLBACK_ROUNDS, source="fallback")
    ...
    for i, round_data in enumerate(rounds_data):
        ...
        if len(round_data["items"]) < 3:
            print(f"[generate-rounds] Round {i} has {len(round_data['items'])} items, too few")
            return GenerateRoundsResponse(rounds=FALLBACK_ROUNDS, source="fallback")
...
async def analyze_response(request: AnalyzeResponseRequest):
    ...
    correct_order = list(range(1, len(request.items) + 1))
...
Return ONLY valid JSON, nothing else."""
    ...
    if not response_text:
        fallback_analysis = {
            ...
            "visualConfusion": {"score": 0.2, "note": "No clear visual confusion patterns detected."},
